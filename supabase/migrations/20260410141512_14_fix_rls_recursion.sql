...
--
-- Problem: the "trips" select policy queries "profiles", and the "profiles"
-- select policy queries "profiles" itself, creating a loop Postgres detects
-- and aborts.
--
...
-- ============================================================================
-- Rewrite the broken policies to use the helper functions instead of
-- subqueries against profiles
-- ============================================================================

-- PROFILES: simplify the select policy so it doesn't reference profiles inside
-- a subquery against profiles
...
create policy "Profiles select agency members"
  on profiles for select using (
    -- Agency admins can see other profiles in their agency
    -- Uses the SECURITY DEFINER helpers to avoid recursion
...
-- PARKS: also rewrite for safety since it also referenced profiles
