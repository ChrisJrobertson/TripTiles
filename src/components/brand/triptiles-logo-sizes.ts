/**
 * Full wordmark (`/images/triptiles-logo.png`).
 * App nav: compact, readable, doesn't dominate the header (~32-40px).
 */
export const TRIP_TILES_LOGO_NAV_IMG_CLASS =
  "h-8 w-auto max-w-[10rem] sm:h-9 md:h-10";

/** Centred auth / wizard — big enough to read, leaves room for the form on phones. */
export const TRIP_TILES_LOGO_AUTH_IMG_CLASS =
  "h-28 w-auto max-w-[min(100%,22rem)] sm:h-32 md:h-40";

/** Footer */
export const TRIP_TILES_LOGO_COMPACT_IMG_CLASS =
  "h-12 w-auto max-w-[14rem] sm:h-16 md:h-20 sm:max-w-[16rem]";

/** Verify / check-email */
export const TRIP_TILES_LOGO_INLINE_IMG_CLASS =
  "h-16 w-auto max-w-[16rem] sm:h-20 md:h-24";
