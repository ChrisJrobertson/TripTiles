...
export function tripReminderEmailHtml(input: TripReminderEmailInput): {
  ...
  const html = `<!DOCTYPE html><html><body style="font-family:Inter,Segoe UI,sans-serif;background:#FAF8F3;color:#0B1E5C;padding:24px;">
  ...
  <p style="margin:0 0 20px;">
    <a href="${escapeHtml(input.tripUrl)}" style="display:inline-block;background:#0B1E5C;color:#FAF8F3;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:600;">View your trip plan →</a>
  </p>
  <p style="margin:24px 0 0;font-size:12px;color:#5c6b8a;line-height:1.5;">
    ...
    <a href="${escapeHtml(unsub)}" style="color:#0B1E5C;">Manage email preferences or unsubscribe from marketing-style emails</a> in Settings.
  </p>
